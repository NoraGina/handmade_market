<?php
class ProductsByStoreController extends AppController
{
    ...
    public function init(){
        ...
        $products = $product->getAllProductsByStoreId($id);
        ...
        if(isset($_SESSION['user']) && isset($_SESSION['cart'])){
            ...
            if(!empty($products) && !empty($categories)){
                 ...
                 $data['cards'] = $this->showProductsByStore($products, $store);
                ...
                $data['storeList']=$this->listOfStores($stores);
                $data['mainContent']="<h2 class='fst-italic text-success text-uppercase'>Hello  $loggedInUser  </h2>";
                $data['mainContent'] .= "<h2 class='fst-italic text-danger '>Nu sunt produse în acest magazin </h2>";
            ...
            if(!empty($products) ){
                $data['title'] = 'Products by Store';
                 ...
                 $data['storeList']=$this->listOfStores($stores);
                 $data['modals']=$this->modals($products);
                 $data['cards'] = $this->showProductsByStorePublic($products, $store);
                 $data['mainContent'] = $this->render(APP_PATH.VIEWS.'mainHomeView.html', $data);
    ...
    //show products by storeId
    public function showProductsByStore($products, $store){
        $name = $store['name'];
        $output="";
        if(is_array($products)){
            $output .="<h3 class= 'text-center text-uppercase fw-lighter'> Produse din magazinul &nbsp; $name </h3>"; 
     ...
     public function showProductsByStorePublic($products, $store){
        
        $output = "";
        if(is_array($products) && is_array($store)){
            $name = $store['name'];
            $output .="<h3 class= 'text-center text-uppercase fw-lighter'> Produse din magazinul &nbsp; $name </h3>"; 
            foreach($products as $product){
            //Taking each  product id
